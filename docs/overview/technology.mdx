...
quickly understand which piece of information is what we are looking for. We believe this to be true also for machine learning
models
...
Using OCR software with something like an NLP component on top of this
imposes several challenges in our opinion.
...
To get satisfactory results when using AI for automation, it needs to be possible to determine when AI predictions
likely are incorrect so that you can discard them or route them to a human for inspection. A major advantage of using
machine learning is that predictions come with an estimate of how likely the prediction is to be correct - and conversely
how likely they are to be incorrect. This estimate is usually called confidence. In designing our models we decided that
it is paramount that confidences we provide are as precise as possible so that they can be trusted. To realise this, it
made sense for us to design our models to be end-to-end, meaning that our models needed to be able to learn everything
on its own by looking at just the pixels in the document and produce the desired outcome with no intermediate steps, handcrafted
...
One of the challenges in training supervised machine learning models is annotating data, usually a time
consuming endeavor. Moreover these annotations are often metadata that you don't really care about - annotations
like bounding boxes, Part-of-Speech tagging and so forth. We wanted to make the training process as smooth as possible
and therefore designed our model to be trainable on data that users actually care about. We came up with a way to train only on actionable data - data that
you are interested in and that are usable to you, without needing any information about where or how this information appears in the document.
Consequently, if you are already processing documents, it will be easier for you to train a good model
because it is much more likely that you have actionable data stored in a SQL database or similar, rather bounding boxes or
Part-of-Speech tags.
...
- Compliance and privacy concerns. Using an OCR API means sending your users' data to a third party. This could introduce
compliance issues and privacy concerns. Another challenge is honouring GDPR's right to be forgotten effectively.
