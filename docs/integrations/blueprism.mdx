...
The validated data from the processed documents are stored in `runs.output.values`, and can be extracted in the
loop iterating runs. Exactly how to extract the data depends on whether or not the field is a line items field or
a "regular" field.

**Extract a "regular" field**

If the data you want to extract is a "regular" field, like `total_amount`, add a *Calculation* to your process.
In Expression, put `[runs.output.values.<YOUR FIELD>]`, where `<YOUR FIELD>` is exchanged with the field you want to
extract (like `total_amount`). Remember to also add a Data object to store the value in. Process a new document in Cradl
(Step 3), run the "Cradl Demo Process" again and see that something has been stored in your newly created Data object.

![Crad Demo Process - extract total_amount](/img/integrations/blueprism/extract_total_amount.png)


**Extract line items fields**
...
for instance want to extract the field `line_items/description`, you should add `description` to Fields.
...
All in all, extracting regular and line items fields will look something like
