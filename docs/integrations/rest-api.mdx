...
After successfully running the flow on a document we can now get the result from the API. When you made the API call to `POST https://api.lucidtech.ai/v1/workflows/<your workflowId>/executions` in step 2 , the response contained an `executionId`. We'll need the `executionId` in addition to the `workflowId` in 
order to get the output from the flow.

The output of the 
flow run contains values for all the fields automatically predicted by Cradl's AI, with potential corrections made by a 
validator. You can now be sure that the extracted data is correct and use it for further 
processing.
