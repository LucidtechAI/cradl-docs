...
After successfully running the flow on a document we can now get the result from the API. When you made the API call to
`POST https://api.lucidtech.ai/v1/workflows/<your workflowId>/executions` in step 2 , the response contained an 
`executionId`. We'll need the `executionId` in addition to the `workflowId` in order to get the output from the flow.
