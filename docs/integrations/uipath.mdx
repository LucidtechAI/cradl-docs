...
<br/>
The Cradl AI integration for UiPath can be downloaded [here](https://www.nuget.org/packages/CradlAI.Activities), and consists of these activities:
...
There is very little setup required in Cradl before you are ready to use the activities from in your UiPath workflows.
...
| flowId        | Input | The identity of the very flow you just created. Store it as a variable in the Data Manager. |
...
We are only listing succeeded runs here, so all runs that are waiting in line to be manually parsed will not show up here.
...
Printing this data in a message box as done in this example is just to illustrate how easy it is to retrieve the output from Cradl and pass it along to other stages in your workflow.
