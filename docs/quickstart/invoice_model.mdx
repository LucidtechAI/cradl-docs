---
sidebar_position: 1
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Training a model
In this guide you will learn how to train a model that is customized to *your* invoices, and extracts the information *you* need.

### Prerequisites
The following prerequisites are required to follow this guide:
1. A Cradl account. If you don't have one, you can sign up [for free](https://cradl.ai).
2. A [*dataset*](/concepts/datasets) of 5000 pairs of invoices and ground truths. See [the next section](#2-gather-training-data) for a concrete example.

## 1. Configure your model
First, we need to create and configure our model.
 1. In Cradl, go to [**Models**](https://app.cradl.ai/models) > [**New model**](https://app.cradl.ai/models).
 2. Give your model a name, and add an optional description.
 3. Add whichever fields you want your model to predict
 
:::tip Field templates
If you need inspiration, you can copy field definitions from one of the provided templates.
:::
 
Foobar

   **Field name**: The name of the field you want to extract. You are free to choose any name you like for your field, but keep in mind that the field names need to match the names in your dataset.

   **Description**: An optional description for the field.

   **Vocabulary**: The set of symbols that is allowed in a prediction of this field. Smaller vocabularies reduces training time.

   **Max length**: The maximum number of symbols that a prediction of this field can contain. Shorter sequences reduces training time.

![Add fields in Cradl](/img/use-cases/fields.png)

## 2. Gather training data
- Prepare training data locally - create pairs of invoices and ground truths as examplified below.


:::info

Download a folder with example invoices and ground truths [here](/sample_invoices.zip)

:::


<Tabs>
<TabItem value="ground-truth" label="invoice-east-repair.json - Ground Truth" default>

```json
[
   {
      "label": "vendor_name",
      "value": "East Repair Inc."
   },
   {
      "label": "total_amount",
      "value": "154.06"
   },
   {
      "label": "due_date",
      "value": "2019-02-26"
   }
]
```

</TabItem>
<TabItem value="document" label="invoice-east-repair.pdf - Invoice">

![invoice-east-repair.pdf](/img/use-cases/invoice-sample-01.png)

</TabItem>
</Tabs>

1. Go to [**Datasets**](https://app.cradl.ai/datasets) > [**New dataset**](https://app.cradl.ai/datasets/new).
2. Go to [**Datasets**](https://app.cradl.ai/datasets) > YourDataset > Upload data.
3. Select files and click **Start upload**.

![upload data](/img/upload-data-app/upload-documents-cradl-1.png)

## 3. Train your model
To make sure that your dataset is compatible with the model we will have to create a [data bundle](../concepts/training-data.mdx)

- Go to **Models > YourModel > Data bundles > Select Data**
- Give your data bundle a name, and add an optional description.
- Choose the dataset you want to train on.
- Choose **Create Data Bundle** and wait for the data bundle summary to appear.
- Inspect the summary to make sure that the training data is of sufficiently good quality to start training.
- Go to **Models > YourModel > Training > New training** and choose the data bundle you created in the previous step.

Congratulations! Your model will now be tailored to your data.
You will be notified when your model is trained and ready to use.
