...
Cradl, we have set a minimum of 100 documents to allow training. In fact, to achieve good results we recommend that 
each label have 100 associated ground truths.

:::info Example
Your datasets contain a total of 100 invoices, of which 8 contain a value for a field called `company_name` in the 
ground truth.
...
contain a`company_name`field, so you now have a total of 108 invoices with`company_name`values. This is a good number 
