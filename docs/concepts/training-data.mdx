...
The process of training a model contains the following steps:
1. User requests training.
...
:::info
We make sure to always provide a model whose architecture is based on the latest research to make sure that our customers get the most out of their data.
:::

## Request a training

The simplest way to request a training is to use the training wizard in the Cradl app, just go to [**Models**](https://app.cradl.ai/models) > **Your model** > **Training jobs** > **New training**.
If you want to request a training with one our SDKs or CLI you need to create a [**Data Bundle**](training-data.mdx#data-bundle) first.
...
A _data bundle_ in Cradl is a model-specific collection of one or more [Datasets](datasets.mdx).
Before training a model, you must specify a data bundle that the model will use as training data.
Since the quality of the trained model depends on the quality of the training data,
we require that any data bundles used for training must pass a statistical test in the form of a [Data Report](training-data.mdx#data-report) before training can begin.

### Creating a data bundle

To create a data bundle for a model you must specify the `modelId` and one or more `datasetIds`, optionally giving it a name and description. See [Creating a model](models.mdx#creating-a-model) and [Creating a dataset](datasets.mdx#creating-a-dataset) for how to create models and datasets.
