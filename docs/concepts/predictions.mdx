...
A prediction in Cradle results from an application of an active, trained [Model](models.mdx) to a [Document](documents.mdx). It supplies the extracted data together with confidence values representing the probability that the extracted data is correct.
